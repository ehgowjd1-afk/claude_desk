...
**디씨씨 자사 작품이 아닙니다.** 사장님 계정으로 소장 중인 타사 작품이라서,

- 파트너스/CP 통계 경로는 쓸 수 없고 일반 독자 계정으로만 접근합니다.
- 결과는 `out/` 의 별도 CSV 로만 남깁니다. `디씨씨이엔티_연재처별_작품현황.xlsx` 의
  리디 시트(자사 작품 16종)에는 섞지 않습니다.
- 댓글이 열리는 범위가 **소장한 회차**로 제한될 수 있습니다. 아래 참고 참조.
